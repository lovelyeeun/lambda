            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                {a.isDefault ? (
                  ...
                  <button
                    onClick={() => { setDefaultShipping(a.id); showToast("기본 배송지로 지정했습니다"); }}
                    className="text-[10px] text-[#999] hover:text-[#3b82f6] cursor-pointer"
                  >
                    기본으로
                  </button>
                )}
              ...
              {a.deliveryNote && (
                <p className="text-[11px] text-[#999] mt-1">요청사항: {a.deliveryNote}</p>
              )}
            </div>
            <div className="flex gap-1 shrink-0">
              <button
                onClick={() => { setEditingId(a.id); setModalMode("edit"); }}
                className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer hover:bg-[#f5f5f5]"
                aria-label="수정"
              >
                <Pencil size={13} strokeWidth={1.5} color="#999" />
              </button>
              <button
                onClick={() => { removeShipping(a.id); showToast("삭제되었습니다"); }}
                className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer hover:bg-[#f5f5f5]"
                aria-label="삭제"
              >
