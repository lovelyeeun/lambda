...
| 12 | ~~승인 알림 체계 설계~~ | ✅ **해결 (v4)**: 슬랙 DM 1순위 → cockpit 앱 내 뱃지 → 이메일 폴백. 긴급 구매는 슬랙 멘션 + 1h 리마인더. 24h 무응답 시 에스컬레이션. |
...
> cockpit은 **별도 제품**으로 GUI를 처음부터 설계. 에어서플라이 DB/API를 백엔드로 활용.
...
**레일 아이콘 순서 = 제품 철학 선언:**
- 채팅을 맨 위 → "이 제품은 대화가 시작점" (cockpit 원래 방향)
...
**에어서플라이 기존 고객:**
- "에어서플라이에서 가져오기" → 기존 폴더 구조 + 상품 + 구매 이력을 cockpit 상품 컨텍스트로 변환
...
cockpit의 상품 컨텍스트는 **회사/팀/개인별로 다른 맥락**을 붙인다.
...
→ cockpit 컨텍스트 레이어는 **에어서플라이 SKU 위에 회사별로 분리된 별도 레이어**로 설계해야 한다.
→ SKU 자체를 수정하는 게 아니라, SKU를 참조하는 cockpit 자체 데이터 모델이 필요.
→ 이 구조가 기술적으로 가능한지, 에어서플라이 상품 API가 어떤 수준까지 노출하는지 확인 필요.

### 권한 체계: RBAC + 뷰 스코프

별도 제품이므로 cockpit 자체 권한 체계.

```
cockpit 조직 셋업 (⚙️ 관리자가 1회)
├ 역할 템플릿 (기본 3개 + 커스텀)
...
| 패턴 | 대표 사례 | 조직 단위 | AI 능동성 | cockpit 차용 |
|------|----------|----------|----------|-------------|
...
| **cockpit** | — | **상품 컨텍스트 + 동적 뷰** | **능동 (패턴 감지 → 제안)** | 세 패턴의 조합 |
...
| # | 질문 | 방향성 메모 |
|---|------|-----------|
| 14 | 에어서플라이 SKU 공유 구조 위에 회사별 컨텍스트 레이어를 어떻게 얹을 것인가? | cockpit 자체 DB에 `product_context` 테이블 설계. SKU ID 참조 + org_id로 분리. |
...
| 17 | cockpit 핀한 뷰(Saved View)와 에어서플라이 기존 폴더의 관계는? | 초기 마이그레이션 시 1:1 뷰 변환. 이후 단방향 참조(cockpit→에어서플라이 SKU)가 유력. |
| 18 | 승인 에스컬레이션 체인 깊이 제한? | 현재 2단계(Buyer→Admin). 대기업 확장 시 3단계 이상 필요할 수 있음. MVP는 2단계. |
| 19 | 슬랙 외 알림 채널 우선순위? | MVP: 슬랙 + cockpit 인앱. 이후: 카카오워크, 팀즈, 이메일. |
